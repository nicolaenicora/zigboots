...
pub fn Range(comptime T: type) type {
    return struct {
        ...
        pub fn iterator(self: *SelfRange) !*RangeIterator {
            return RangeIterator.init(self);
        }
        /// Initialize. Returns error if step size is invalid.
        pub fn init(allocator: Allocator, start: T, end: T, step: T) !SelfRange {
            if (step == 0) {
                return error.ZeroStepSize;
            }

            return SelfRange{ .allocator = allocator, .start = start, .end = end, .step = step };
        }

        const RangeIterator = struct {
            const SelfRangeIterator = @This();

            target_self: *SelfRange,
            ...
            /// Initialize. Returns error if step size is invalid.
            pub fn init(source: *SelfRange) !*SelfRangeIterator {
